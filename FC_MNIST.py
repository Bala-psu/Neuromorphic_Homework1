...
def modified_sigmoid(x):
    sig = 1 / (1 + torch.exp(-1*x)) #Making the slope a little bit sharper
    return sig
...
def train(args, model, device, train_loader,optimizer, epoch, batch_size, label_features = 100):
    ...
    train_accu, train_losses, run_acc, run_loss = [], [], [], []
    ...
    LOSSES_ = run_loss
    ACCURACY = run_acc
    np.save(os.path.join('/home/mdl/bus44/Neuromorphic Computing/runs/Q2/', 'TRAIN_acc_'+ str(epoch)), ACCURACY)
    np.save(os.path.join('/home/mdl/bus44/Neuromorphic Computing/runs/Q2/', 'TRAIN_loss_'+ str(epoch)), LOSSES_)
...
def main():
    ...
    for trial in range(1,args.nTrials+1):
        ...
        np.save(os.path.join('/home/mdl/bus44/Neuromorphic Computing/runs/Q2/', 'TEST_acc_'+ str(epoch)), TEST_acc)
